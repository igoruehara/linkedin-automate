...
# Projeto — Curador de Perfil (LinkedIn)

> Preenchido no `/kickoff` (2026-06-20). Detalhe nos docs linkados — aqui é o resumo operacional.
> Extensão de navegador **open source** que cura o perfil do LinkedIn aberto: Pontuação + Checklist
> + Reescrita (copiar/colar). Ver [vision](docs/product/vision.md) e [overview](docs/architecture/overview.md).

## Stack
- **Linguagem:** TypeScript · **Runtime/alvo:** extensão de navegador Manifest V3.
- **Framework de extensão:** **WXT** (sobre Vite) · **UI:** **React** (side panel).
- **Navegadores:** Chrome/Brave (MVP) → Firefox (Onda 2) → Safari (Onda 3).
- **IA:** porta de LLM plugável, **default Claude (Anthropic)**, modelo `claude-opus-4-8` ou
  `claude-sonnet-4-6` para reescrita · **BYOK** (chave local do usuário).
- **Sem backend** — ver [ADR-0002](docs/architecture/adr/0002-extensao-cross-browser-wxt-react.md) ·
  [ADR-0004](docs/architecture/adr/0004-motor-hibrido-regras-mais-ia.md) ·
  [ADR-0005](docs/architecture/adr/0005-byok-privacidade-cliente.md).

## Camadas (DDD) neste projeto
- `domain/` — Perfil, Seção, Regra, Achado, Pontuação, Recomendação + portas (Extração, LLM). Sem DOM/SDK.
- `application/` — casos de uso: curar perfil, reescrever seção, configurar chave.
- `infrastructure/` — ACL de extração do DOM (seletores), cliente de LLM (BYOK), storage local.
- `interfaces/` — content script, service worker, side panel (React). Ver [src/](src/README.md).

## Comandos
> Ainda **a definir** no scaffolding (1ª feature). Convenção alvo com WXT + pnpm:
- `pnpm dev` — dev com HMR no navegador · `pnpm build` / `pnpm zip` — build/empacota por loja.
- `pnpm test` — Vitest (unit) · `pnpm test:e2e` — Playwright · `pnpm lint` / `pnpm format`.
- Gate da esteira: `node scripts/audit-esteira.mjs .` (roda no CI `esteira.yml`).

## Quality gates
- **Vitest** nas Regras/Pontuação (domínio puro) — todo AC-N da spec mapeado a teste.
- **Fixtures de HTML** (PT/EN) versionadas para a Extração (ver [ADR-0003](docs/architecture/adr/0003-extracao-via-dom-com-acl.md)).
- **Playwright** E2E carregando a extensão num navegador real.
- **ESLint + Prettier** obrigatórios · regra de dependência das camadas respeitada.
- Detalhe e comandos canônicos: [docs/engineering/TESTING.md](docs/engineering/TESTING.md).

## Observabilidade & privacidade (inegociável)
- **Zero telemetria por padrão.** Nada de uso/erro sai do navegador; logs estruturados só em dev.
- O **Perfil** é processado em memória, no cliente. A **Chave de IA** vive só no storage local.
- Única saída de rede: o **Provedor de LLM** do usuário, na Reescrita — explícito na UI.
- **Nunca** automatize ações no LinkedIn — só leitura do DOM visível (fronteira de ToS).
