...
| 4  | Coletar **fixtures** PT/EN (completo, sem-sobre, sobre-vazio, sobre-recolhido, com/sem-foto) **+ golden `*.expected.json`** (oráculo dos valores) | AC-1..6 | — | revisão + `pnpm test` | **done (calibradas c/ HTML real)** |
| 5  | ACL `ExtratorDomLinkedin`: seletores + detecção de idioma (PT/EN → chaves canônicas) | AC-1,2 | 1,2,3,4 | `pnpm test` | **done (calibrado c/ HTML real)** |
