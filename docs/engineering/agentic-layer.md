...
## Estado neste projeto (Curador de Perfil) — `/camada-agentica` em 2026-06-20

### Gerado
- **Rules:** `CLAUDE.md` preenchido (stack/camadas/gates/privacidade) · `.claude/settings.json`
  com **allowlist** dos comandos do stack (pnpm build/test, `pnpm exec vitest|playwright`,
  `node scripts/*.mjs`) e git **somente-leitura**. Hook `SessionStart` preservado.
- **Agents** (`.claude/agents/`):

  | Agente | Papel | Insumo que o motiva |
  |---|---|---|
  | `spec-reviewer` | Gate DoR (Given/When/Then testável, não-objetivos, glossário) | Esteira SDD |
  | `vitest-tester` | Roda/interpreta Vitest + Playwright; mapeia AC→teste | Qualidade · `TESTING.md` |
  | `dom-extraction-auditor` | Diagnostica seletor quebrado vs fixtures; confina o conserto no ACL | Risco DOM volátil · [ADR-0003](../architecture/adr/0003-extracao-via-dom-com-acl.md) |
  | `privacy-guard` | Revisa diff contra invariantes de privacidade/ToS | [ADR-0005](../architecture/adr/0005-byok-privacidade-cliente.md) |
  | `adr-writer` | Rascunha ADR no formato Nygard | Fluxo ADR-pesado |
  | `domain-modeler` | Extrai linguagem ubíqua/agregados; checa glossário | DDD · 4 bounded contexts |

### Adiado → roadmap de adoção
- **Hook `Stop`** (lint + Vitest ao encerrar) → criar **junto com o scaffolding** (não há `package.json` ainda).
- **CI/CD** de build/test → skill **`/setup-ci`**.
- **Skills tools-aware** (`/spec-to-jira`, `/publicar-confluence`) → exigem MCP; rodar **`/integracoes`** antes.
- **Skills do produto** (`/nova-regra` p/ adicionar Regra de curadoria, `/atualizar-fixtures`) → após o scaffolding.
